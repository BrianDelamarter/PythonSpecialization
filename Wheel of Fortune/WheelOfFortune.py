...
class WOFPlayer:
    def __init__(self, name, balance = 0, gifts = None):
        self.name = name
        self.prizeMoney = balance
        if gifts == None:
            self.prizes = []
        else:
            self.prizes = gifts
    ...
    def __str__(self):
        return ("{a} (${b})".format(a = self.name, b = self.prizeMoney))
...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        print("""{a} has ${b}
                ...
                Category: {c}
                Phrase:  {d}
                Guessed: {e}""".format(a = self.name,b = self.prizeMoney,c = category,d = obscuredPhrase,e = guessed))
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, name, difficulty= 5, balance=0, gifts=[]):
        super().__init__(name, balance, gifts)
        self.difficulty = difficulty
    
    def smartCoinFlip(self):
        intelligence = random.randint(1,10)
        if intelligence >= self.difficulty:
            return False
        else:
            return True
            ...
            if letter not in guessed:
                if letter in VOWELS: 
                    if self.prizeMoney >= VOWEL_COST:
                        possibleGuesses.append(letter)
                else:
                    possibleGuesses.append(letter)          
        return possibleGuesses
        
    def getMove(self, category, obscuredPhase, guessed):
        options = self.getPossibleLetters(guessed)
        ...
        if smart:
            ...
            x = random.randint(1,len(options)-1)
            return options[x] 
